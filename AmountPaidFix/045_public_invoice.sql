...
--
-- Fixed: sales has no amount_paid column — POS always inserts a
-- sale with status:'paid', there is no partial/credit tracking in
-- the current checkout flow, so every invoice is fully paid by
-- definition. Removed the column instead of guessing a replacement.
--
...
RETURNS TABLE (
  ...
  gst_amount NUMERIC, discount NUMERIC, total NUMERIC,
