...
const NAV = [
  ...
  { id:'leaves',        label:'Leave Mgmt',        icon:'🗓️' },
    ...
    <div style={{ display:'flex', height:'100vh', background:T.bg, color:T.ink, fontFamily:'"DM Sans",system-ui,-apple-system,sans-serif', overflow:'hidden' }}>
      ...
      <div className="sidebar-wrap" style={{
        ...
        <div style={{ padding: sideOpen ? '12px 14px' : '12px 0', borderBottom:`1px solid ${T.bdr}`, display:'flex', alignItems:'center', gap:10, justifyContent: sideOpen ? 'flex-start' : 'center', flexShrink:0 }}>
          <div style={{ width:34, height:34, borderRadius:9, flexShrink:0, background:`linear-gradient(135deg,${T.blue},${T.purple})`, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:900, fontSize:14, color:'#fff', boxShadow:`0 0 16px ${T.blue}44` }}>ES</div>
          {sideOpen && <div style={{ overflow:'hidden' }}>
            <div style={{ fontWeight:800, fontSize:12, color:T.ink, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', maxWidth:150, letterSpacing:'-0.01em' }}>{tenant?.name || 'Elite Store'}</div>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'6px 5px' }}>
          {NAV.map((item, i) => {
            if (item.section) {
              if (!sideOpen) return <div key={i} style={{ height:1, background:T.bdr, margin:'6px 4px' }} />;
              return (
                <div key={i} style={{ padding:'10px 9px 4px', fontSize:9, fontWeight:700, color:T.muted, textTransform:'uppercase', letterSpacing:'0.08em' }}>{item.section}</div>
              );
            }
            ...
            return (
              <button key={item.id + i}
                ...
                style={{
                  ...
                  padding: sideOpen ? '7px 9px' : '9px 0',
        ...
        <div style={{ borderTop:`1px solid ${T.bdr}`, padding:'8px 5px', flexShrink:0 }}>
          {sideOpen && user && (
            <div style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 9px', marginBottom:4 }}>
              <div style={{ width:30, height:30, borderRadius:7, background:`linear-gradient(135deg,${T.blue},${T.purple})`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:800, color:'#fff', flexShrink:0 }}>
      ...
      {/* Mobile overlay */}
      {mobile && mobOpen && <div onClick={() => setMobOpen(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.6)', zIndex:399 }} />}

      {/* ── Main ── */}
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>

        {/* Topbar */}
        <div style={{ height:48, background:T.srf, borderBottom:`1px solid ${T.bdr}`, display:'flex', alignItems:'center', padding:'0 14px', gap:10, flexShrink:0, position:'relative', zIndex:10 }}>
          ...
          {!online && <div style={{ background:T.amber+'18', border:`1px solid ${T.amber}44`, borderRadius:6, padding:'2px 9px', fontSize:10, color:T.amber }}>📴 Offline</div>}
          ...
          {tenant?.plan_status === 'trial' && (
            <div onClick={() => onNavigate('billing')} style={{ background:T.amber+'18', border:`1px solid ${T.amber}44`, borderRadius:6, padding:'3px 10px', fontSize:10, color:T.amber, cursor:'pointer' }}>
              Trial → Upgrade
            </div>
          )}
        </div>
