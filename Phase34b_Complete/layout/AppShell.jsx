...
import { filterNav, ROLES } from '../../lib/roleAccess';
...
export default function AppShell({ children, user, tenant, page, onNavigate, onLogout, branches, activeBranch, onBranchChange, online = true, role = 'owner' }) {
  ...
  const { online: netOnline, pending, syncing, lastSync, cachedAt, sync } = useOffline(tenant);
  const isOnline = netOnline && online;
    ...
    <div style={{ display:'flex', height:'100vh', background:BG, color:INK, fontFamily:'"DM Sans",system-ui,-apple-system,sans-serif', overflow:'hidden' }}>
      <style>{`
        *{box-sizing:border-box}
        ...
        /* ── RESPONSIVE: applies to all pages without editing them ── */
        ...
        @media (max-width: 900px) {
          ...
          div[style*="380px 1fr"], div[style*="1fr 380px"],
          div[style*="360px 1fr"], div[style*="1fr 360px"],
          div[style*="320px 1fr"], div[style*="1fr 300px"],
          div[style*="300px 1fr"], div[style*="280px 1fr"],
          div[style*="1fr 400px"], div[style*="400px 1fr"] { grid-template-columns: 1fr !important; }
        }
        @media (max-width: 768px) {
          div[style*="padding:24px"] { padding: 14px !important; }
          div[style*="repeat(5,1fr)"],
          div[style*="repeat(4,1fr)"],
          div[style*="repeat(3,1fr)"] { grid-template-columns: repeat(2, 1fr) !important; gap: 10px !important; }
          div[style*="repeat(2,1fr)"] { grid-template-columns: 1fr !important; }
          div[style*="minmax(320px"], div[style*="minmax(330px"],
          div[style*="minmax(340px"], div[style*="minmax(400px"],
          div[style*="minmax(280px"], div[style*="minmax(230px"] { grid-template-columns: 1fr !important; }
          table { display: block; overflow-x: auto; white-space: nowrap; -webkit-overflow-scrolling: touch; }
          table thead, table tbody { display: table; width: 100%; min-width: 620px; }
          form div[style*="repeat(4,1fr)"],
          form div[style*="repeat(3,1fr)"],
          ...
          {sideOpen && user && (
            <div style={{ display:'flex', alignItems:'center', gap:9, padding:'8px 10px', marginBottom:6, background:'rgba(255,255,255,.07)', borderRadius:8 }}>
              ...
              <div style={{ overflow:'hidden', flex:1 }}>
                ...
                <div style={{ fontSize:9, color:'rgba(255,255,255,.65)' }}>{ROLES[role]?.icon} {ROLES[role]?.label || 'Owner'}</div>
              </div>
            </div>
          )}
          {!mobile && (
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>
        ...
        <div style={{ height:52, background:WHITE, borderBottom:`1px solid ${BDR}`, display:'flex', alignItems:'center', padding:'0 20px', gap:12, flexShrink:0, boxShadow:'0 1px 4px rgba(0,0,0,.06)' }}>
          ...
          {!isOnline && <div style={{ background:'#FFFBEB', border:'1px solid #FDE68A', borderRadius:6, padding:'2px 9px', fontSize:10, color:'#D97706' }}>📴 Offline</div>}
          <div style={{ flex:1 }} />
          ...
          <OfflineBanner online={isOnline} pending={pending} syncing={syncing} lastSync={lastSync} cachedAt={cachedAt} onSync={sync}/>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:5, background: isOnline ? '#F0FDF4' : '#FFFBEB', border:`1px solid ${isOnline?'#BBF7D0':'#FDE68A'}`, borderRadius:20, padding:'3px 10px' }}>
            <div style={{ width:6, height:6, borderRadius:'50%', background: isOnline ? '#16A34A' : '#D97706' }} />
            <span style={{ fontSize:10, fontWeight:600, color: isOnline ? '#16A34A' : '#D97706' }}>{isOnline ? 'Live' : 'Offline'}</span>
          </div>
        </div>
