...
import CommissionRun     from './CommissionRun';
...
const TABS = [
  ...
  { id:'commrun',     label:'Commission Run', icon:'💵' },
  { id:'commissions', label:'Commission Log', icon:'📑' },
  { id:'scheduler',   label:'Scheduler',      icon:'📆' },
  { id:'dashboard',   label:'HR Dashboard',   icon:'👥' },
];

export default function HRHub({ tenant, deepTab, role = 'owner' }) {
  const [tab, setTab] = useState('qr');
  const visibleTabs = filterTabs(TABS, role, 'hrhub');
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      ...
      <div style={{ flex:1, overflow:'auto', background:'#F7F3F3' }}>
        ...
        {tab==='commrun'     && <CommissionRun     tenant={tenant}/>}
