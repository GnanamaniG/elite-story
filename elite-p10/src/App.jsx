...
function LoginPage() {
  ...
  async function handle(e){
    ...
    try{
      if(mode==='login'){const{error:err}=await signIn(email,password);if(err)throw err;}
      else{if(!bizName.trim()){setError('Business name required');setLoading(false);return;}const{error:err}=await signUp(email,password,{biz_name:bizName,biz_type:'retail'});if(err)throw err;setError('✅ Account created! Check your email.');setMode('login');}
      ...
      <div style={{width:'100%',maxWidth:400}}>
        ...
        <div style={{background:T.srf,border:`1px solid ${T.bdr}`,borderRadius:14,padding:28}}>
          ...
          <div style={{display:'flex',background:T.bg,borderRadius:9,padding:3,marginBottom:20}}>
            {[['login','Sign In'],['signup','Create Account']].map(([id,label])=>(<button key={id} onClick={()=>{setMode(id);setError('');}} style={{flex:1,background:mode===id?T.srf:'transparent',color:mode===id?T.ink:T.sub,border:mode===id?`1px solid ${T.bdr}`:'none',borderRadius:7,padding:'8px',fontSize:13,fontWeight:mode===id?700:500,cursor:'pointer',fontFamily:'inherit'}}>{label}</button>))}
          </div>
          <form onSubmit={handle} style={{display:'flex',flexDirection:'column',gap:13}}>
            {mode==='signup'&&<div><label style={{fontSize:11,color:T.sub,fontWeight:700,textTransform:'uppercase',display:'block',marginBottom:5}}>Business Name *</label><input value={bizName} onChange={e=>setBizName(e.target.value)} placeholder="e.g. Signals Elite" style={inp}/></div>}
            <div><label style={{fontSize:11,color:T.sub,fontWeight:700,textTransform:'uppercase',display:'block',marginBottom:5}}>Email *</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@business.com" style={inp} required/></div>
            <div><label style={{fontSize:11,color:T.sub,fontWeight:700,textTransform:'uppercase',display:'block',marginBottom:5}}>Password *</label><input type="password" value={password} onChange={e=>setPassword(e.target.value)} style={inp} required/></div>
            <button type="submit" disabled={loading} style={{background:T.blue,color:'#fff',border:'none',borderRadius:9,padding:'13px',fontSize:15,fontWeight:700,cursor:'pointer',fontFamily:'inherit',marginTop:4}}>{loading?'Please wait…':mode==='login'?'Sign In':'Create Account'}</button>
          </form>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    supabase.from('branches').select('*').eq('tenant_id',activeTenant.id).eq('active',true)
      ...
      .then(({data})=>{if(data?.length){setBranches(data);setActiveBranch(data.find(b=>b.is_main)||data[0]);}})
      .catch(()=>{});
  },[activeTenant?.id]);
  ...
  return(
    <>
      ...
      <AppShell tenant={activeTenant} user={user} page={page} onNavigate={setPage} onLogout={()=>signOut()} branches={branches} activeBranch={activeBranch} onBranchChange={setActiveBranch}>
