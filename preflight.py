...
def unchecked_error_scan(root):
    """Flags Promise.all([...supabase queries]) where a destructured
    result's .data is read but .error never is — the exact pattern
    that broke customer search in POS. Warning-level: doesn't fail
    the build, but the report should not be ignored."""
    import re, glob
    warnings = []
    for f in glob.glob(f'{root}/pages/*.jsx'):
        c = open(f).read()
        name = f.split('/')[-1]
        for m in re.finditer(r'const \[([^\]]+)\]\s*=\s*await Promise\.all\(\[', c):
            varlist = [v.strip() for v in m.group(1).split(',')]
            window = c[m.end():m.end()+1200]
            uses_checkerrors_helper = 'checkErrors(' in window
            for var in varlist:
                if not var: continue
                if uses_checkerrors_helper: continue  # correctly handled via the shared helper
                if f'{var}.data' in window and f'{var}.error' not in window:
                    warnings.append((name, var))
    return warnings
...
if __name__ == '__main__':
    ...
    warnings = unchecked_error_scan(root)
    if warnings:
        by_file = {}
        for name, var in warnings: by_file.setdefault(name, []).append(var)
        print(f"\n⚠️  {len(warnings)} unchecked query result(s) across {len(by_file)} file(s) — same pattern that broke POS customer search.")
        print(f"   Not a build blocker, but worth fixing. Top offenders:")
        for name, vars_ in sorted(by_file.items(), key=lambda x: -len(x[1]))[:10]:
            print(f"     {name:28} {len(vars_)} unchecked")
