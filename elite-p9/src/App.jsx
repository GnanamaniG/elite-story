...
function LoginPage() {
  ...
  async function sendOTP() {
    ...
    try {
      ...
      const { error: err } = await signIn.auth.signInWithOtp({ phone: formatted });
      if (err) throw err;
      setOtpSent(true);
      ...
      <div style={{ width:'100%', maxWidth:400 }}>
        ...
        <div style={{ background:T.srf, border:`1px solid ${T.bdr}`, borderRadius:14, padding:28 }}>
          ...
          <form onSubmit={handle} style={{ display:'flex', flexDirection:'column', gap:13 }}>
            {mode === 'signup' && (
              <div>
                <label style={{ fontSize:11, color:T.sub, fontWeight:700, textTransform:'uppercase', display:'block', marginBottom:5 }}>Business Name *</label>
                <input value={bizName} onChange={e => setBizName(e.target.value)} placeholder="e.g. Signals Elite" style={inp} />
              </div>
            )}
            <div>
              <label style={{ fontSize:11, color:T.sub, fontWeight:700, textTransform:'uppercase', display:'block', marginBottom:5 }}>Email *</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@business.com" style={inp} required />
            </div>
            <div>
              <label style={{ fontSize:11, color:T.sub, fontWeight:700, textTransform:'uppercase', display:'block', marginBottom:5 }}>Password *</label>
