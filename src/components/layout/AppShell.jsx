...
// ── Premium White + Maroon Design System ──────────────────────
...
const NAV = [
  ...
  { id:'salesdochub',  label:'Sales Documents',    icon:'📄' },
    ...
    <div style={{ display:'flex', height:'100vh', background:BG, color:INK, fontFamily:'"DM Sans",system-ui,-apple-system,sans-serif', overflow:'hidden' }}>
      <style>{`
        *{box-sizing:border-box}
        ...
        button:focus-visible{outline:1.5px solid #C0392B;outline-offset:2px}
        ...
        .skel{background:linear-gradient(90deg,#F0E8E8 25%,#F8F0F0 50%,#F0E8E8 75%);background-size:200% 100%;animation:skelShine 1.4s ease-in-out infinite;border-radius:6px}
        ...
        @media(max-width:768px){.hide-mobile{display:none!important}.tbl-wrap{overflow-x:auto}}
      `}</style>
      ...
      <div style={{
        ...
        <nav style={{ flex:1, overflowY:'auto', padding:'8px 8px' }}>
          {NAV.map((item, i) => {
            if (item.section) {
              ...
              return (
                <div key={i} style={{ padding:'14px 8px 4px', fontSize:9, fontWeight:800, color:SIDEBAR_SEC, textTransform:'uppercase', letterSpacing:'0.1em' }}>
                  {item.section}
                </div>
              );
            }
            ...
            return (
              <button key={item.id + i}
                ...
                style={{
                  ...
                  gap:9, padding: sideOpen ? '8px 10px' : '10px 0',
                  justifyContent: sideOpen ? 'flex-start' : 'center',
                  background: active ? SIDEBAR_ACT : 'transparent',
                  color: active ? SIDEBAR_ATXT : SIDEBAR_TXT,
        ...
        <div style={{ borderTop:`1px solid rgba(255,255,255,.1)`, padding:'10px 8px', flexShrink:0 }}>
          ...
          <button onClick={onLogout}
            ...
            onMouseEnter={e => { e.currentTarget.style.background='rgba(255,100,100,.2)'; e.currentTarget.style.color='#FF8A80'; e.currentTarget.style.borderColor='rgba(255,100,100,.3)'; }}
            onMouseLeave={e => { e.currentTarget.style.background='transparent'; e.currentTarget.style.color='rgba(255,255,255,.6)'; e.currentTarget.style.borderColor='rgba(255,255,255,.15)'; }}>
            🚪 {sideOpen && 'Sign Out'}
          </button>
        </div>
      </div>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>
        ...
        <div style={{ height:52, background:WHITE, borderBottom:`1px solid ${BDR}`, display:'flex', alignItems:'center', padding:'0 20px', gap:12, flexShrink:0, boxShadow:'0 1px 4px rgba(0,0,0,.06)' }}>
          <button onClick={() => mobile ? setMobOpen(s=>!s) : setCollapsed(s=>!s)}
            style={{ background:'none', border:'none', color:SUB, cursor:'pointer', fontSize:18, lineHeight:1, padding:'4px' }}>
            ☰
          </button>
          ...
          {tenant?.plan_status==='trial' && (
            <div onClick={()=>onNavigate('billing')} style={{ background:'#FEF2F2', border:'1px solid #FECACA', borderRadius:6, padding:'4px 12px', fontSize:11, color:RED, cursor:'pointer', fontWeight:600 }}>
              Upgrade Plan ›
            </div>
          )}
        </div>
