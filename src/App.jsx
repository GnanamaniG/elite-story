...
function LoginPage() {
  ...
  async function handle(e) {
    ...
    try {
      if (mode==='login') {
        const {error:err}=await signIn(email,password); if(err)throw err;
      } else {
        if(!bizName.trim()){setError('Business name required');setLoading(false);return;}
      ...
      <div style={{width:'100%',maxWidth:400}}>
        ...
        <div style={{background:T.srf,border:`1px solid ${T.bdr}`,borderRadius:14,padding:28}}>
          ...
          <form onSubmit={handle} style={{display:'flex',flexDirection:'column',gap:13}}>
            ...
            <button type="submit" disabled={loading} style={{background:T.blue,color:'#fff',border:'none',borderRadius:9,padding:'13px',fontSize:15,fontWeight:700,cursor:'pointer',fontFamily:'inherit',marginTop:4}}>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    supabase.from('branches').select('*').eq('tenant_id',activeTenant.id).eq('active',true)
      ...
      .then(({data})=>{
        if(data?.length){setBranches(data);setActiveBranch(data.find(b=>b.is_main)||data[0]);}
      }).catch(()=>{});
  },[activeTenant?.id]);

  if(loading)return(
    <div style={{minHeight:'100vh',background:T.bg,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:16}}>
      <div style={{width:52,height:52,background:T.blue,borderRadius:14,display:'flex',alignItems:'center',justifyContent:'center',fontWeight:900,fontSize:20,color:'#fff'}}>ES</div>
      <div style={{width:40,height:40,border:'3px solid #4f7cff',borderTopColor:'transparent',borderRadius:'50%',animation:'spin .7s linear infinite'}}/>
